...
[
  ...
  num-green-in-giant ;; number of green nodes in the giant component
  ...
  prob-blue-green ;; probability for blue-green connections
  prob-red-green ;; probability for red-green connections
  RED-GCC
  BLUE-GCC
  GREEN-GCC
]
...
;; Creates edges based on probabilities for all node color pairs
to add-edges
  let red-nodes turtles with [color = red]
  let blue-nodes turtles with [color = blue]
  let green-nodes turtles with [color = green]

  ;; Check if there are any red nodes
  if any? red-nodes [
    ;; Create edges between red nodes
    create-edges red-nodes red-nodes prob-red-red

    ;; Create edges between red and blue nodes if there are blue nodes
    if any? blue-nodes [
      create-edges red-nodes blue-nodes prob-red-blue
      create-edges blue-nodes red-nodes prob-blue-red
    ]

    ;; Create edges between red and green nodes if there are green nodes
    if any? green-nodes [
      create-edges red-nodes green-nodes prob-red-green
      create-edges green-nodes red-nodes prob-green-red
    ]
  ]

  ;; Check if there are any blue nodes
  if any? blue-nodes [
    ;; Create edges between blue nodes
    create-edges blue-nodes blue-nodes prob-blue-blue

    ;; Create edges between blue and green nodes if there are green nodes
    if any? green-nodes [
      create-edges blue-nodes green-nodes prob-blue-green
      create-edges green-nodes blue-nodes prob-green-blue
    ]
  ]

  ;; Check if there are any green nodes
  if any? green-nodes [
    ;; Create edges between green nodes
    create-edges green-nodes green-nodes prob-green-green
  ]
end


to create-edges [nodes1 nodes2 prob]
  let node1 one-of nodes1
  let node2 one-of nodes2
  ask node1 [
  if node1 != node2 and not link-neighbor? node2 [
