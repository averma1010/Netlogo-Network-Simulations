...
to setup
  ...
  set prob-unlink unlink_probability
  make-turtles
...
to find-all-components
  ...
  set giant-component-size 0 ;; reset giant-component-size before finding components
...
to add-edge
  let node1 one-of turtles with [color = red]
  let node2 one-of turtles with [color = red and self != node1]
  ask node1 [
  ;; Ensure nodes are different
  if (node1 != node2) [
    if (link-neighbor? node2) [
      ;; They have a link, consider breaking it with probability prob-unlink
      if (random-float 1 <= prob-unlink) [
        ask link-with node2 [ die ]
      ]
    ]
    if ( not link-neighbor? node2) [
      ;; They do not have a link, consider creating it with probability prob-red-red
      if (random-float 1 <= prob-red-red) [
       create-link-with node2 [ set color red ]
      ]
    ]
  ]]
end
...
"" ""
PENS
"default" 1.0 0 -8053223 true "" "plotxy (ticks)\n       (giant-component-size )"
